...
const FreelancerDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const appsRes = await axios.get(`${process.env.REACT_APP_API_URL}/applications/freelancer/${user.id}`);
  ...
  const onSubmitApplication = async (data) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_API_URL}/applications/${selectedJob._id}/apply`, {
